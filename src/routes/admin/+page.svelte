...
<div class="min-h-screen bg-stone-50 py-8 px-4 sm:px-6 lg:px-8">
  <div class="max-w-5xl mx-auto">
    ...
    {#if form?.success}
      <div class="mb-6 p-4 bg-green-50 border border-green-200 text-green-800 rounded-md text-sm flex items-center">
    ...
    {#if form?.error}
      <div class="mb-6 p-4 bg-red-50 border border-red-200 text-red-800 rounded-md text-sm flex items-center">
    ...
    <!-- Add Form -->
    {#if showAddForm}
      <div class="bg-white shadow-md rounded-lg p-6 mb-8 border border-stone-200">
        <h3 class="text-xl font-medium text-stone-800 mb-4 flex items-center">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 text-stone-600" viewBox="0 0 20 20" fill="currentColor">
        ...
        <form method="POST" action="?/add" use:enhance enctype="multipart/form-data">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
            <div>
              <label for="name" class="block text-sm font-medium text-stone-700 mb-1">Item Name</label>
              <input 
                type="text" 
                id="name" 
                name="name" 
            ...
            <div>
              <label for="pageNumber" class="block text-sm font-medium text-stone-700 mb-1">Page Number</label>
              <input 
                type="number" 
                id="pageNumber" 
                name="pageNumber" 
            ...
            <!-- Image Upload Field -->
            <div class="md:col-span-2">
              <label for="image" class="block text-sm font-medium text-stone-700 mb-1">Food Image</label>
              <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                <div class="flex-1 w-full">
                  <input 
                    ...
                    class="w-full px-3 py-2 border border-stone-300 rounded-md shadow-sm focus:outline-none focus:ring-stone-500 focus:border-stone-500 text-stone-800"
                  />
                  <p class="mt-1 text-xs text-stone-500">Recommended size: 400x400px, max 2MB</p>
                </div>
                {#if imagePreview}
                  <div class="flex-shrink-0 relative">
                    <img src={imagePreview || "/placeholder.svg"} alt="Preview" class="h-16 w-16 object-cover rounded-md border border-stone-300" />
                  </div>
                {/if}
              </div>
            </div>
    ...
    {#if showEditForm && editingItem}
      <div id="edit-form" class="bg-white shadow-md rounded-lg p-6 mb-8 border border-stone-200">
        <h3 class="text-xl font-medium text-stone-800 mb-4 flex items-center">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 text-stone-600" viewBox="0 0 20 20" fill="currentColor">
        ...
        <form method="POST" action="?/edit" use:enhance enctype="multipart/form-data">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
            <div>
              <label for="edit-name" class="block text-sm font-medium text-stone-700 mb-1">Item Name</label>
              <input 
                type="text" 
                id="edit-name" 
                name="name" 
            ...
            <div>
              <label for="edit-price" class="block text-sm font-medium text-stone-700 mb-1">Price ($)</label>
              <input 
                type="number" 
                id="edit-price" 
                name="price" 
            ...
            <div>
              <label for="edit-category" class="block text-sm font-medium text-stone-700 mb-1">Category</label>
              <select 
                id="edit-category" 
                name="category" 
                required 
              ...
              >
                {#each categories as category}
                  <option value={category} selected={editingItem.category === category}>{category}</option>
                {/each}
              </select>
            </div>
            
            <div>
              <label for="edit-pageNumber" class="block text-sm font-medium text-stone-700 mb-1">Page Number</label>
              <input 
                type="number" 
                id="edit-pageNumber" 
                name="pageNumber" 
            ...
            <div class="md:col-span-2">
              <label for="edit-description" class="block text-sm font-medium text-stone-700 mb-1">Description</label>
              <textarea 
                id="edit-description" 
                name="description" 
                rows="2" 
            ...
            <!-- Image Upload Field -->
            <div class="md:col-span-2">
              <label for="edit-image" class="block text-sm font-medium text-stone-700 mb-1">Food Image</label>
              <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                <div class="flex-1 w-full">
                  <input 
                    type="file" 
    ...
    <!-- Menu Items Table -->
    <div class="bg-white shadow-md rounded-lg border border-stone-200 overflow-hidden">
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-stone-200">
            <thead class="bg-stone-50">
              <tr>
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Item</th>
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Price</th>
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Category</th>
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Page</th>
                  ...
                  <td class="px-6 py-4">
                    <div class="flex items-center">
                      <div class="h-12 w-12 flex-shrink-0">
                        {#if item.image}
                          <img src={item.image || "/placeholder.svg"} alt={item.name} class="h-12 w-12 object-cover rounded-md border border-stone-200" />
                        {:else}
                          <div class="h-12 w-12 bg-stone-100 rounded-md flex items-center justify-center text-stone-400">
                      ...
                      <div class="ml-4">
                        <div class="text-sm font-medium text-stone-800 {item.outOfStock ? 'line-through opacity-70' : ''}">{item.name}</div>
                        <div class="text-xs text-stone-500 truncate max-w-xs">{item.description}</div>
                      </div>
                    </div>
                  </td>
                  <td class="px-6 py-4 text-sm text-stone-500 {item.outOfStock ? 'line-through opacity-70' : ''}">${item.price.toFixed(2)}</td>
                  <td class="px-6 py-4">
                    <span class="px-2 py-1 text-xs rounded-full bg-stone-100 text-stone-800">
                      {item.category}
                    </span>
                  </td>
                  <td class="px-6 py-4 text-sm text-stone-500">{item.pageNumber}</td>
                  <td class="px-6 py-4">
                    ...
                    <div class="flex justify-end gap-2">
                      ...
                      <form method="POST" action="?/toggleStock" class="inline-block" use:enhance>
                        ...
                        <button 
                          ...
                          title={item.outOfStock ? 'Mark as available' : 'Mark as out of stock'}
                        >
                          {#if item.outOfStock}
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      ...
                      {#if confirmDeleteId === item.id}
                        <div class="absolute right-16 mt-1 bg-white shadow-lg rounded-md border border-stone-200 p-3 z-10">
                          <p class="text-sm text-stone-700 mb-2">Delete this item?</p>
